...
public:
  explicit Pentropy4(const ActionOptions&);
  ~Pentropy4();
// active methods:
...
PLUMED_REGISTER_ACTION(Pentropy4,"PENTROPY4")

void Pentropy4::registerKeywords( Keywords& keys ){
...
Pentropy4::Pentropy4(const ActionOptions&ao):
 ...
 for(unsigned int i=rank;i<nn;i+=stride) {   
  ...
  for(unsigned int j=(minBin+1);j<maxBin+1;j+=1) {   
	...
	value = pairing((xmin-distanceModulo)/sqrt2sigma, dfunc);
        gofr[j-1] += value;
        gofr[j] -= value;
	valueDer =	invSqrt2sigmaDistanceVersor*dfunc;
        gofrPrime[j-1][i0] += valueDer;
        gofrPrime[j-1][i1] -= valueDer;
        gofrPrime[j][i0] -= valueDer;
        gofrPrime[j][i1] += valueDer;
        Tensor vv(valueDer,distance);
        gofrVirial[j-1] += vv;
        gofrVirial[j] -= vv;
  }
  // First bin
  ...
  gofr[minBin] -= value;
  valueDer =	invSqrt2sigmaDistanceVersor*dfunc;
  gofrPrime[minBin][i0] -= valueDer;
  gofrPrime[minBin][i1] += valueDer;
  Tensor vv1(valueDer,distance);
  gofrVirial[minBin] -= vv1;
  // Last bin
  xmin = (maxBin+1)*deltar;
  value = pairing((xmin-distanceModulo)/sqrt2sigma, dfunc);
  gofr[maxBin] += value;
  valueDer =	invSqrt2sigmaDistanceVersor*dfunc;
  gofrPrime[maxBin][i0] += valueDer;
  gofrPrime[maxBin][i1] -= valueDer;
  Tensor vv2(valueDer,distance);
  gofrVirial[maxBin] += vv2;
 }
...
double Pentropy4::pairing(double distance,double&dfunc)const{
