...
public:
  explicit Pentropy3(const ActionOptions&);
  ~Pentropy3();
// active methods:
...
PLUMED_REGISTER_ACTION(Pentropy3,"PENTROPY3")

void Pentropy3::registerKeywords( Keywords& keys ){
...
Pentropy3::Pentropy3(const ActionOptions&ao):
...
const unsigned nn=nl->size();
 ...
 entropy *= -2*pi*density; // Integral pre-factor
  ...
  for(unsigned int i=rank;i<getNumberOfAtoms();i+=stride) {
    ...
    for(unsigned int j=0;j<nhist;j+=1) {   
      if (gofr[j]>1.e-10) {
        deriv[i] += (gofrPrime[j][i]*logGofr[j])*pow(deltar*j,2);
      }
    }
...
double Pentropy3::pairing(double distance,double&dfunc)const{
